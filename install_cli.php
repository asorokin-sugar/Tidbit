...
require_once('Tidbit/StorageDriver/Factory.php');
require_once('Tidbit/InsertObject.php');
...
// zero means use default value provided by storage driver
$insertBatchSize = 0;
...
$storageFactory = new Tidbit_StorageDriver_Factory($storageType, $storage, $logQueriesPath, $insertBatchSize);
...
foreach ($module_keys as $module) {
    ...
    if (in_array($module, $moduleUsingGenerators)) {
        ...
        $generator = new $generatorName($GLOBALS['db'], $storageFactory);
    ...
    echo "\n\tHitting DB... ";
    $beanStorageDriver = $storageFactory->getDriver();
    $relationStorageDrivers = array();
    ...
    for ($i = 0; $i < $total; $i++) {
        ...
        $insertObject = new Tidbit_InsertObject($dTool->table_name, $dTool->installData);
        $beanStorageDriver->saveByChunk($insertObject);

        if ($dTool->getRelatedModules()) {
            foreach ($dTool->getRelatedModules() as $table => $installDataArray) {
                if (empty($relationStorageDrivers[$table])) {
                    $relationStorageDrivers[$table] = $storageFactory->getDriver();
                }
                foreach ($installDataArray as $data) {
                    $relationInsertObject = new Tidbit_InsertObject($table, $data);
                    $relationStorageDrivers[$table]->saveByChunk($relationInsertObject);
                }
            }
        }
        ...
        if ($_GLOBALS['txBatchSize'] && $i % $_GLOBALS['txBatchSize'] == 0) {
            $beanStorageDriver->commitQuery();
            echo "#";
        }
        if ($i % 1000 == 0) echo '*';
    } //for

    echo microtime_diff($beanStorageDriver->getQueryExecStartTime(), microtime()) . "s ";
    ...
    if ($module == 'Teams') {
        ...
        $teamGenerator = new Tidbit_Generator_TeamSets($GLOBALS['db'], $storageFactory, $generatedIds);
    ...
    if ($module == 'ACLRoles' && !empty($_SESSION['tba'])) {
        ...
        $tbaGenerator = new Tidbit_Generator_TBA($GLOBALS['db'], $storageFactory, $generatedIds);
...
if (!empty($_SESSION['as_populate'])) {
    ...
    if (!empty($GLOBALS['beanList']['Activities'])) {
        ...
        $tga = new Tidbit_Generator_Activity($storageFactory);
        $asModules = array();
...
if ($storageType == 'csv') {
    ...
    $converter = new Tidbit_CsvConverter($GLOBALS['db'], $storage);
