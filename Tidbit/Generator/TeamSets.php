...
class Tidbit_Generator_TeamSets extends TeamSet
{
     ...
     */
    protected $storageTeamSets;

    /**
     * @var Tidbit_StorageDriver_Storage_Abstract
     */
    protected $storageTeamSetsTeams;
     ...
     */
    // stopping here -- create 2 factors
    public function __construct(DBManager $db, Tidbit_StorageDriver_Factory $storageFactory, $teamIds)
    {
        $this->db = $db;
        $this->storageTeamSets = $storageFactory->getDriver();
        $this->storageTeamSetsTeams = $storageFactory->getDriver();
        $this->teamIds = $teamIds;
        $this->storageTypeDb = $storageFactory->getStorageType() != Tidbit_StorageDriver_Factory::OUTPUT_TYPE_CSV;
        $this->loadTeamIds();
        $this->loadTeamMd5();
    }
    ...
    private function addTeamsToCreatedTeamSet($teams)
    {
        ...
        if (!in_array($team_md5, $this->teamMd5Array)) {
            ...
            $insertObject = new Tidbit_InsertObject('team_sets', $installData);
            $this->storageTeamSets->saveByChunk($insertObject);
            ...
            foreach ($teams as $team_id) {
                ...
                $insertObject = new Tidbit_InsertObject('team_sets_teams', $installData);
                $this->storageTeamSetsTeams->saveByChunk($insertObject);
